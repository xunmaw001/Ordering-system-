...
//                            caipinOrderEntity.setCanzhuoId(Integer.valueOf(data.get(0)));   //餐桌 要改的
...
//                            caipinOrderEntity.setCaipinOrderTruePrice(data.get(0));                    //实付价格/积分 要改的
    ...
    */
    ...
    public R add(@RequestBody CaipinOrderEntity caipinOrder, HttpServletRequest request){
        ...
        if("用户".equals(role)){
            ...
            caipinOrder.setCaipinOrderTruePrice(caipinEntity.getCaipinNewMoney()*caipinOrder.getBuyNumber()); //设置实付价格/积分
    ...
    public R add(@RequestParam Map<String, Object> params, HttpServletRequest request){
        ...
        //查询当前用户下是否有已占用的餐桌
        CanzhuozhanyongEntity canzhuozhanyongEntity = canzhuozhanyongService.selectOne(new EntityWrapper<CanzhuozhanyongEntity>().eq("yonghu_id", userId).eq("canzhuozhanyong_types", 1));
        if(canzhuozhanyongEntity == null){
            return R.error("您还未占用餐桌,先占用餐桌过来结算哦");
        }
        ...
        for (Map<String, Object> map : caipins) {
           //取值
            ...
            caipinOrderEntity.setCanzhuoId(canzhuozhanyongEntity.getCanzhuoId());//餐桌
            ...
            if(caipinOrderPaymentTypes == 1){//余额支付
                ...
                Double money = caipinEntity.getCaipinNewMoney() * buyNumber;
                if(yonghuEntity.getYonghuNewJifen() - money <0 ){
                    ...
                    caipinOrderEntity.setCaipinOrderTruePrice(money);//实付积分
                }
